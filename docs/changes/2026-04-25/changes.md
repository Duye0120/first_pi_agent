...
改了什么：
- `diff-panel.tsx`：恢复工具栏里的横向对比 / 竖向对比切换按钮。
- `diff-panel.tsx`：单列文件列表继续保留，展开的文件 diff 按当前对比方向渲染。
- `diff-panel.tsx`：将对比方向写回 diff 面板草稿状态，重新打开面板时沿用上次选择。

为什么改：
- 用户确认需要保留 diff 内容内部的横向 / 竖向对比能力；去掉文件树双列后，这个能力仍然属于单个文件 diff 的阅读控制。
